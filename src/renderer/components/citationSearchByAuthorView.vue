<template>
  <v-layout row wrap>
    <v-flex xs12 class='mb-3'>
      <h1 class="headline">저자명 기준 검색</h1>
      <v-divider></v-divider>
    </v-flex>
    <!-- 옵션 컨테이너 -->
    <v-flex xs6>
...
export default {
  props: ['aResList', 'aErrQuery', 'loading', 'executer', 'log'],
  data() {
    return {
      errObj: {
        show: false,
  ...
  methods: {
    ...
    deleteRow(index) {
      for (let ii = 0; ii < this.aResList.length; ii += 1) {
        if (this.aResList[ii].index === index) {
          index = ii;
          break;
        }
      }
      if (confirm('정말 행을 삭제합니까?')) {
        this.aResList.splice(index, 1);
      }
    },
    stdin() {
      // 인풋값 체크
      ...
      const payload = {
        scope: this,
        inputs: ['citationSearchByAuthor', this.query, this.startYear, this.endYear, this.pAuthors, this.organizaion],
      };
