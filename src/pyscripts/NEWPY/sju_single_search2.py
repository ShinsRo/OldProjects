...
class ChromeDriver():
    ...
    def get_page_source_from_url(self, url):
        self.driver.get(url)
        return self.driver.page_source
        
    def get_page_source_from_http_res(self, filename, http_res):
        cwd = os.getcwd()
        with open(cwd + '/%s.html'%filename, 'w+', encoding='utf-8') as _temp:
            _temp.write(http_res.text)
        
        self.driver.get('File://%s/_temp.html'%cwd)
        return self.driver.page_source
...
class SingleSearch():
    ...
    def start(self, query, start_year, end_year, gubun):
        ...
        #     )
        #     return
        # target_content = driver.get_page_source_from_http_res(filename='_temp', http_res=http_res)
        
        target_content = driver.get_page_source_from_url(reffer)
        ...
        target_content = driver.get_page_source_from_url(url)
